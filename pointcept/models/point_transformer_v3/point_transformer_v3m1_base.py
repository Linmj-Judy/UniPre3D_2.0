...
class PointTransformerV3(PointModule):
    ...
    def forward(
        ...
        Args:
            data_dict (dict): Dictionary containing input point cloud data
            img_features (list): List of image features from different scales
            links (tensor): Linking information between 2D and 3D features
            unprojected_coords (tensor, optional): Unprojected coordinates for 2D-3D fusion
            fusion_mlps (nn.Module): MLPs for point fusion

        Returns:
            Point: Processed point cloud with updated features
        """
        ...
        # Update point coordinates and features
        data_dict["coord"] = original_point.coord
        original_point.feat = original_point.sparse_conv_feat.features
        original_point.offset = torch.tensor(
            [len(original_point.batch)], device=original_point.batch.device
        )

        # Copy relevant keys from original point to new dictionary
        for key in data_dict.keys():
            if key in data_dict:
                now_data_dict[key] = original_point[key]
        now_data_dict["batch"] = original_point.batch

        # Create new point instance and process
        point = Point(now_data_dict)
        ...
        # Forward through decoder if not in classification mode
        if not self.cls_mode:
