...
from .Mamba3D_utils import misc
from .Mamba3D_utils.checkpoint import (
...
from ..backbone.pointmlp import PointNetFeaturePropagation
...
class GroupFeature(nn.Module):  # FPS + KNN
    ...
    def forward(self, xyz, feat):
        ...
        neighborhood = neighborhood.view(
            batch_size, num_points, self.group_size, 3
        ).contiguous()  # 1 128 8 3
        neighborhood_feat = feat.contiguous().view(-1, C)[
            idx, :
        ]  # BxNxK C 128x8 384   128*26*8
        assert neighborhood_feat.shape[-1] == feat.shape[-1]
        neighborhood_feat = neighborhood_feat.view(
            batch_size, num_points, self.group_size, feat.shape[-1]
        ).contiguous()  # 1 128 8 384
        # normalize
        neighborhood = neighborhood - center.unsqueeze(2)
...
class K_Norm(nn.Module):
    ...
    def forward(self, lc_xyz, lc_x):
        ...
        # Feature Expansion
        knn_x = torch.cat(
            [knn_x, lc_x.reshape(B, G, 1, -1).repeat(1, 1, K, 1)], dim=-1
        )  # B G K 2C
...
class Mamba3DBlock(nn.Module):
    ...
    def forward(self, center, x):
        ...
        x = x + self.drop_path(
            self.lfa(center, self.norm1(x))
        )  # x: 32 129 384. center: 32 128 3
...
class Mamba3DEncoder(nn.Module):
    ...
    def forward(
        ...
        """
        fusion_start_idx = [len(self.blocks) - 1]
        i = 0
        # TODO: pre-compute knn (GroupFeature)
        for idx, block in enumerate(self.blocks):
            x = block(center, x + pos)
            if idx in fusion_start_idx:
                x = feature_fusion(
                    x, center, image_features, c2w_projection_matrix, intrinsic
                )
                i += 1
        return x
...
class Mamba3DSeg(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.pos_embed = nn.Sequential(
            nn.Linear(3, 128), nn.SiLU(), nn.Linear(128, self.trans_dim)
        )
    ...
    def forward(
        self, pts, image_features, c2w_projection_matrix, feature_mlps, intrinsic
    ):
